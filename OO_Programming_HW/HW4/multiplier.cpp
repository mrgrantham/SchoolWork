...
enum operation { ADDITION, SUBTRACTION, MULTIPLICATION, NO_OPERATION };
...
int main(int argc, char** argv) {
    ...
    bool BIN_FLAG1 = true;
    bool BIN_FLAG2 = true;
    bool MULTIPLIER_FLAG = false;
    
        
    bool SET_REG1_SIZE_FLAG = false;
    bool SET_REG2_SIZE_FLAG = false;
    bool SET_REG1_VALUE_FLAG = false;
    bool SET_REG2_VALUE_FLAG = false;
        ...
        switch(command) {
            ...
            case 'o':
                ...
                if(*optarg == '-'){
                    ...
                    parameters.push_back(SUBTRACTION);

                } else if(*optarg == '+'){
                    operation = ADDITION;
                    parameters.push_back(ADDITION);

                } else if(*optarg == '*' || *optarg == 'x'){
                    MULTIPLIER_FLAG = true;
                    operation = MULTIPLICATION;
                    parameters.push_back(MULTIPLICATION);
    ...
    if(SET_REG1_SIZE_FLAG){
        aRegister.shiftRegSize(shiftBits);
    }
    if(SET_REG2_SIZE_FLAG && !MULTIPLIER_FLAG){
        bRegister.shiftRegSize(shiftBits2);
        // cout << "bRgister size: " << bRegister.getSize() << endl;
        // cout << "bRegister shiftBits2" << shiftBits2 << endl;
    }
    if(MULTIPLIER_FLAG){
        // cout << "Multiplier_flag called" << endl;
        // cout << "aRegister size: " << aRegister.getSize() << endl;

        bRegister.shiftRegSize(aRegister.getSize());
    }    
    ...
    for (int i = 0; i < commands.size(); i++) {
        switch(commands[i]) {
            ...
            case 'R':
                ...
                break;
            case 'o':
                if(parameters[i] == SUBTRACTION)
                    newRegister = aRegister - bRegister;
                else if(parameters[i] == ADDITION)
                    newRegister = aRegister + bRegister;
                else if(parameters[i] == MULTIPLICATION)
                    newRegister = aRegister * bRegister;
                break;
            default:
...
// RegisterOverload Class
    ...
    RegisterOverload RegisterOverload::operator*(const RegisterOverload& aRegister) {
        RegisterOverload Result;
        RegisterOverload Multiplicand = *this;
        RegisterOverload Multiplier = aRegister;
        Result.setPrintSettings(printSettings);

        Result.shiftRegSize(Multiplicand.getSize() + Multiplier.getSize());
        Multiplicand.shiftRegSize(Result.getSize());
       
        bool MULTIPLIER_IS_NOT_ZERO = true;
        bool NEGATIVE_MULTIPLIER = Multiplier.registerBits[Multiplier.getSize()-1];
        int PRODUCT_REGISTER_SIZE = Result.getSize();
        // cout << "PRODUCT SIZE: " << Result.getSize() << endl;

        for(int product_digit = 0; product_digit < PRODUCT_REGISTER_SIZE ; product_digit++){
            
            // cout << "MULTIPLIER: " << Multiplier << endl;
            // cout << "MULTIPLICAND: " << Multiplicand << endl;
            
            // Test if multiplier is still not 0
            
            /*
            // Test is multiplier is negative
            if(NEGATIVE_MULTIPLIER){
                
                // If negative check if -1 before shift
                for(int i = 0;i < Multiplier.getSize(); i++){
                    MULTIPLIER_IS_NOT_ZERO = MULTIPLIER_IS_NOT_ZERO && Multiplier.registerBits[i];
                }
                MULTIPLIER_IS_NOT_ZERO = !MULTIPLIER_IS_NOT_ZERO;
            } 
            */
                
            // Check if lest significant bit is 1(odd) or 0(even)
            if(Multiplier.registerBits[0])
            ...
            // Halve Multiplier
            Multiplier.setShiftValue(Multiplier.registerBits[Multiplier.getSize()-1]); // Set to 1 if negative or 0 if positive
            Multiplier.shiftRight(1);        

            /*
            if(!NEGATIVE_MULTIPLIER) {
                
                // If positive check if zero after shift
                for(int i = 0;i < Multiplier.getSize(); i++)
                    MULTIPLIER_IS_NOT_ZERO = MULTIPLIER_IS_NOT_ZERO || Multiplier.registerBits[i];
            }
            */
